  ...
  try {
    if (options.claudeApiKey) {
      ...
      try {
        ...
        cliOutput(`${chalk.yellow('Note:')} Verification test failed, but the key has been saved. You can test it when using Claude API features.\n`);
      }
      } else {
      ...
      if (apiKey) {
        ...
        cliOutput(`Set it using: ${chalk.cyan('npx @auth0/auth0-mcp-server config --claude-api-key <your-key>')}\n`);
      }
    }
  } catch (error) {
